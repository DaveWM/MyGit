    ...
    public class RepoTests : TestBase
    {
        ...
        public new void Init()
        {
            ...
            GitHubClientMock.Setup(m => m.Activity.Starring.CheckStarred(It.IsAny<string>(), It.IsAny<string>()))
                .Returns(() => Task.FromResult(false));
            GitHubClientMock.Setup(m => m.PullRequest.GetForRepository(It.IsAny<string>(), It.IsAny<string>())).Returns(() => Task.FromResult(new List<PullRequest>() as IReadOnlyList<PullRequest>));
            GitHubClientMock.Setup(m => m.Issue.GetForRepository(It.IsAny<string>(), It.IsAny<string>())).Returns(() => Task.FromResult(new List<Issue>() as IReadOnlyList<Issue>));
        ...
        public async void TestRepoPRViewModel()
        {
            ...
            GitHubClientMock.Setup(m => m.PullRequest.GetForRepository(Owner, Repo)).Returns(() => Task.FromResult(new List<PullRequest> { new PullRequest()} as IReadOnlyList<PullRequest>));
            await vm.Refresh();
            Assert.AreEqual(1, vm.PullRequests.Count());
            GitHubClientMock.Verify(m => m.PullRequest.GetForRepository(Owner,Repo), Times.Once());
        }
        ...
        public async void TestRepoIssuesVM()
        {
            ...
            GitHubClientMock.Setup(m => m.Issue.GetForRepository(Owner, Repo))
                .Returns(() => Task.FromResult(new List<Issue> {new Issue()} as IReadOnlyList<Issue>));
            await vm.Refresh();
            Assert.AreEqual(1, vm.Issues.Count());
            GitHubClientMock.Verify(m => m.Issue.GetForRepository(Owner,Repo));
        }
